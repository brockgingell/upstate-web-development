...
export function SiteHeader() {
  ...
  return (
    <header> {isMenuOpen ? 
      <>
        <div className="fixed top-0 z-50 flex w-screen flex-col border-b-2 bg-background px-2">
          <div className="relative top-0 flex h-16 items-center justify-start">
            <NavLogo />
          </div>
        ...
        <button className='relative top-16 z-40 flex h-screen justify-start border-t-2' onClick={() => setIsMenuOpen(false)}>
          <MobileNavLinks items={siteConfig.mainNav} />
        </button>
      </>
      :
      <>
        <div className="fixed top-0 z-50 flex h-16 w-screen flex-row items-center justify-between border-b-2 bg-background px-2">
          <NavLogo />
